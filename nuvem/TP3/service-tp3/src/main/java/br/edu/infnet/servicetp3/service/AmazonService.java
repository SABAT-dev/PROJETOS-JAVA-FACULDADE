...
public class AmazonService {
    ...
    private void iniciarS3() {
        ...
        s3aws = AmazonS3ClientBuilder.standard().withRegion(region).withCredentials(awsStaticCredentialsProvider).build();
    }

    public List<String> listar() {

        List<String> nameArchives = new ArrayList<String>();
        ObjectListing objectListing = s3aws.listObjects(bucket);
        List<S3ObjectSummary> objectSummaries = objectListing.getObjectSummaries();

        if (objectSummaries != null) {
            for (S3ObjectSummary s3obj : objectSummaries) {
                String nameArchiver = s3obj.getKey();
                nameArchives.add(nameArchiver);
            }
        }
        return nameArchives;
    }
